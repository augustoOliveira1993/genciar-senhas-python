...
def mostrar_por_id(id_serv: int):
    deletado = cursor.execute(
        f"""SELECT id_alter, service FROM gerencia_senha WHERE id_alter={id_serv}""")
...
while True:
    ...
    if op == 'R':
        service = input(
            'Qual o serviço para o qual quer a senha? ').strip().upper()
        get_pasword(service)

    if op == 'A':
        service = input(
            'De qual servico deseja alterar senha: ').strip().upper()
